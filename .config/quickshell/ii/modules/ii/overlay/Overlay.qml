...
Scope {
    ...
    GlobalShortcut {
        name: "overlayNotesToggle"
        description: "Toggles the notes widget on press"

        onPressed: {
            if (OverlayContext.notesOnly) {
                GlobalStates.overlayOpen = false;
            } else {
                if (!Persistent.states.overlay.open.includes("notes")) {
                    Persistent.states.overlay.open.push("notes");
                }
                OverlayContext.notesOnly = true;
    ...
    Connections {
        ...
        function onOverlayOpenChanged() {
            if (!GlobalStates.overlayOpen) {
                OverlayContext.notesOnly = false;
            }
        }
    }
}
